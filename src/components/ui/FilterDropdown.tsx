    ...
    >
      <div className="flex items-center justify-between pb-2 mb-2 border-b" style={{ borderColor: 'var(--border)' }}>
        <div>
          ...
          <p className="text-[10px]" style={{ color: 'var(--text-muted)' }}>Select filters to narrow the results</p>
        </div>
        {onClear && hasActiveFilters && (
          <button type="button" onClick={onClear} className="inline-flex items-center gap-1 text-[10px] font-semibold text-blue-600 hover:text-blue-700">
      ...
      {search && (
        <div className="mb-2 relative">
          ...
          <input type="search" value={search.value} onChange={(event) => search.onChange(event.target.value)}
      ...
      <div className="space-y-2">
        {filters.map((filter) => {
          const isActive = filter.active ?? (filter.value !== '' && filter.value !== 'all' && filter.value !== 'All');
          return (
            <label key={filter.key} className="block rounded-lg border p-2.5 transition-colors"
              style={{ borderColor: isActive ? '#93c5fd' : 'var(--border)', background: isActive ? 'rgba(37,99,235,0.04)' : 'var(--bg-subtle)' }}>
              <span className="flex items-center gap-1.5 mb-1.5 text-[10px] font-bold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>
                {isActive && <Check className="h-3 w-3 text-blue-600" />}{filter.key}
              </span>
              {filter.type === 'date' ? (
                <input type="date" value={filter.value} onChange={(event) => filter.onChange(event.target.value)}
